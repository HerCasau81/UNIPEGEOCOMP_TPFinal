...
# Descarga radios_2022

```{r}
# --- CARGA DEL CSV DE ATRIBUTOS (Parte I) ---
# Asegúrate de que esta línea apunte al CSV que contiene las variables estadísticas
personas <- read_csv("Indicadores de personas. Radios, 2022 - Córdoba.csv") 
...
```{r}
# ============================================================
# ANÁLISIS GEOESTADÍSTICO - CENSO 2022 (Córdoba Capital)
# ============================================================
...
# ------------------------------------------------------------
# 1 CARGA Y LIMPIEZA DE DATOS ESTADÍSTICOS (personas)
# ------------------------------------------------------------
...
df_atributos_limpio <- personas %>%
  rename(
    COD_RAD_CENSAL_RAW = `Código de radio.`
  ) %>%
  select(
    COD_RAD_CENSAL_RAW,
    POB_TOTAL = `Población total (en hogares familiares).`,
    POB_JVEN  = `Población de 18 a 29 años.`,
    POB_TOTAL_GENERAL = `Población total`
  ) %>%
  mutate(
    # Crear clave completa de 9 dígitos
    COD_RAD_CENSAL = str_pad(as.character(COD_RAD_CENSAL_RAW), width = 9, side = "left", pad = "0"),
    COD_PROV = str_sub(COD_RAD_CENSAL, 1, 2),
    COD_DPTO = str_sub(COD_RAD_CENSAL, 3, 5)
  ) %>%
  # Filtramos Córdoba Capital (14014)
  filter(COD_PROV == "14" & COD_DPTO == "014")
...
# ------------------------------------------------------------
# 3 GEOMETRÍA (SHAPEFILE DE RADIOS)
# ------------------------------------------------------------
...
ggplot(sf_final_mapa_listo) +
  geom_sf(aes(fill = DENSIDAD), color = NA) +
  scale_fill_viridis_c(option = "magma", trans = "log10", labels = scales::comma) +
  labs(
    title = "Densidad de Población por Radio Censal",
    subtitle = "Departamento Capital (Córdoba)",
    fill = "Habitantes / km²"
...
#bins <- 10^c(0, 1, 2, 3, 4, 5)
#pal <- colorBin("magma", domain = sf_final_mapa_listo$DENSIDAD, bins = bins)

bins <- quantile(sf_final_mapa_listo$DENSIDAD, probs = seq(0, 1, length.out = 8), na.rm = TRUE)
pal <- colorBin("magma", domain = sf_final_mapa_listo$DENSIDAD, bins = bins)
...
mapa_leaflet <- leaflet(sf_final_mapa_listo) %>%
  ...
  addLegend(pal = pal, values = ~DENSIDAD, title = "Habitantes / km²", opacity = 0.7) %>%
  addControl(html = "<b>Densidad de Población por Radio Censal<br>Córdoba Capital (Censo 2022)</b>",
             position = "topleft")
